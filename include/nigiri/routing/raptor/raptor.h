...
struct raptor {
  ...
  raptor(
      ...
      : tt_{tt},
        ...
        n_rt_routes_{Rt ? rtt->n_rt_routes() : 0U},
  ...
  void execute(unixtime_t const start_time,
    ...
    for (auto k = 1U; k != end_k; ++k) {
      ...
      state_.station_mark_.for_each_set_bit([&](std::uint64_t const i) {
        ...
        if constexpr (Rt) {
          // Transports identical to schedule are on the static scan above and
          // are on neither list, so they cost nothing here. Grouped ones are
          // scanned once per group; the rest -- additional trips, reroutings,
          // cancelled stops and runs -- keep their own scan.
          for (auto const& rt_r :
