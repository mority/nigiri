...
struct raptor {
  ...
  void execute(unixtime_t const start_time,
    ...
    //                start_time, delta_to_unix(base(),
    //                state_.round_times_[k][i]), location{tt_,
    //                location_idx_t{i}}, k - 1);
...
private:
  ...
  void update_footpaths(unsigned const k, profile_idx_t const prf_idx) {
    ...
    //      auto const& fps = kFwd ?
    //      tt_.locations_.footpaths_out_[prf_idx][l_idx]
    //                             :
    ...
    //                to_unix(state_.tmp_[to_idx(l_idx)]), fp.duration(),
    //                location{tt_, fp.target()},
    //                state_.best_[to_idx(fp.target())], fp_target_time,
    //                lower_bound, to_unix(clamp(fp_target_time +
    //                dir(lower_bound))), to_unix(time_at_dest_[k]));
    //            continue;
    //          }
    //
    //          trace_upd(
