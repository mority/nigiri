...
void tb_preprocessing::build_transfer_set() {
  ...
  {
    auto const timer = scoped_timer("build_transfer_set");

#pragma omp parallel default(none) shared(ts_, progress_tracker)
    {
                  ...
                  while (omega.any() && sigma_w <= sigma_w_max_) {
                    ...
                    if (req) {
                      ...
                      auto const sigma =
                          ...
                          static_cast<int>(sigma_t.v_ + sigma_fp.v_ +
                                           sigma_w.v_);
