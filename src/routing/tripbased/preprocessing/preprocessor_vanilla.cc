...
void preprocessor::build_part(route_idx_t const route_t) {
  ...
  // iterate transports of route r
  for (auto const t : tt_.route_transport_ranges_[route_t]) {
    ...
    for (auto i = static_cast<stop_idx_t>(1U); i != stop_seq_t.size(); ++i) {
      ...
      for (auto const& fp : tt_.locations_.footpaths_out_[p_t_i]) {
        rr_arr_.update_walk(fp.target(), tau_arr_t_i + fp.duration_,
                            fp.duration_, beta_t, nullptr);
        rr_ch_.update_walk(fp.target(), tau_arr_t_i + fp.duration_,
                           fp.duration_, beta_t, nullptr);
      }
      ...
      rr_ch_.update(p_t_i,
                    ...
                    beta_t, nullptr);
      for (auto const& fp : tt_.locations_.footpaths_out_[p_t_i]) {
        rr_arr_.update(fp.target(), tau_arr_t_i + fp.duration_, beta_t,
                       nullptr);
