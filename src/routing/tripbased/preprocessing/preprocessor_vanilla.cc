...
void preprocessor::build_part(transport_idx_t const t) {
  ...
  rr_ch_.reset();
  // reverse iteration
  for (auto i = static_cast<stop_idx_t>(stop_seq_t.size() - 1U); i != 0U; --i) {
#else
  for (auto i = static_cast<stop_idx_t>(1U); i != stop_seq_t.size(); ++i) {
    ...
    // transferring from
    std::int8_t const sigma_t = tt_.event_mam(t, i, event_type::kArr).days_;
...
#else
    rr_arr_.update(p_t_i, tau_arr_t_i, beta_t, nullptr);
    rr_ch_.update(p_t_i,
                  tau_arr_t_i + tt_.locations_.transfer_time_[p_t_i].count(),
                  beta_t, nullptr);
    for (auto const& fp : tt_.locations_.footpaths_out_[p_t_i]) {
      rr_arr_.update(fp.target(), tau_arr_t_i + fp.duration_, beta_t, nullptr);
      rr_ch_.update(fp.target(), tau_arr_t_i + fp.duration_, beta_t, nullptr);
    }
#endif
#endif

    auto handle_fp = [&sigma_t, &t, &i, &route_t, &alpha,
#ifdef TB_PREPRO_TRANSFER_REDUCTION
...
#endif
#endif
                      &omega, &theta, &beta_t, this](footpath const& fp) {
      ...
      auto const tau_q_d = static_cast<int8_t>(tau_q / 1440U);
      ...
      for (auto const route_u : routes_at_q) {
        ...
        for (stop_idx_t j = 0U; j < tt_.route_location_seq_[route_u].size() - 1;
             ++j) {
          ...
          if ((p_u_j == q) &&
              stop{tt_.route_location_seq_[route_u][j]}.in_allowed()) {
            ...
            while (omega.any()) {
              ...
              std::uint16_t const tau_dep_alpha_u_j =
                  static_cast<std::uint16_t>(sigma) * 1440U + tau_dep_u_j->mam_;
              ...
              auto const req =
                  ...
                  (u != t &&
                   (tau_dep_alpha_u_j -
                        (tt_.event_mam(u, j, event_type::kDep).count() -
                         tt_.event_mam(u, i, event_type::kArr).count()) <
                    alpha));
                ...
                if (theta.any()) {
                  ...
                  auto const check_uturn = [&j, &route_u, &i, &route_t,
                    ...
                    if (j + 1 < tt_.route_location_seq_[route_u].size() &&
                      ...
                      // stop
                      if (p_u_next.location_idx() == p_t_prev.location_idx() &&
                        ...
                        std::uint16_t const tau_arr_alpha_t_prev =
                            ...
                            static_cast<std::uint16_t>(
                                tt_.event_mam(t, i, event_type::kArr).count() -
                                tt_.event_mam(t, i - 1, event_type::kArr)
                                    .count());
                        auto const min_change_time = static_cast<std::uint16_t>(
                            tt_.locations_
                  ...
                  if (!check_uturn()) {
                    ...
                    for (stop_idx_t l = j + 1U;
                      ...
                      std::uint16_t const tau_arr_t_u_l =
                          tau_dep_t_u_j +
                          static_cast<std::uint16_t>(
                              tt_.event_mam(u, l, event_type::kArr).count() -
                              tau_dep_u_j->count());
                      auto const p_u_l =
                      ...
                      for (auto const& fp_r :
                        ...
                        rr_ch_.update_walk(fp_r.target(), eta, walk_time_r,
                                           theta, &impr);
#elifdef TB_TRANSFER_CLASS
                        rr_arr_.update_class(fp_r.target(), eta, kappa, theta,
                                             &impr);
                        rr_ch_.update_class(fp_r.target(), eta, kappa, theta,
                                            &impr);
#else
                        rr_arr_.update(fp_r.target(), eta, theta, &impr);
                        rr_ch_.update(fp_r.target(), eta, theta, &impr);
#endif
                      }
                    }
