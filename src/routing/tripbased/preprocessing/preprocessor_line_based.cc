...
void preprocessor::build_part(route_idx_t const current_route) {
  ...
  neighborhood.reserve(route_max_length_ * 10);
  ...
  line_transfers(current_route, neighborhood);

  // the stops of the current route
  auto const stop_seq_from = tt_.route_location_seq_[current_route];

  // get transports of current route
  auto const& route_transports = tt_.route_transport_ranges_[current_route];
  ...
  for (auto const t : route_transports) {
    ...
    if (!neighborhood.empty()) {
      ...
      auto stop_seq_to = tt_.route_location_seq_[route_to_prev];
      ...
      for (auto const& neighbor : neighborhood) {
        ...
        if (route_to_prev != neighbor.route_idx_to_) {
          ...
          stop_seq_to = tt_.route_location_seq_[route_to_prev];
#ifdef TB_MIN_WALK
          rlb.reset_walk(stop_seq_to.size());
#elifdef TB_TRANSFER_CLASS
          rlb.reset_class(stop_seq_to.size());
        ...
        auto const tau_arr_t_i =
            tt_.event_mam(t, neighbor.stop_idx_from_, event_type::kArr);
...
#ifndef NDEBUG
        TBDL << "Transport " << t << " arrives at source stop: " << dhhmm(alpha)
             << ", earliest possible departure at target stop: " << dhhmm(tau_q)
             << "\n";
#endif
        ...
        auto tau_dep_u_j =
            std::lower_bound(event_times.begin(), event_times.end(), tau_q_tod,
        ...
        while (omega.any()) {
          ...
          auto const tau_dep_alpha_u_j =
              static_cast<std::uint16_t>(sigma_fpw * 1440 + tau_dep_u_j->mam_);
          ...
          auto const u =
              tt_.route_transport_ranges_[neighbor.route_idx_to_]
          ...
          auto const& beta_u = tt_.bitfields_[tt_.transport_traffic_days_[u]];
          ...
          if (theta.any()) {
            ...
            auto const otid =
                compute_otid(sigma_t + sigma_fpw - sigma_u,
                             tt_.event_mam(u, 0, event_type::kDep).mam_);
#endif
            ...
            rlb.update(neighbor.stop_idx_to_, sigma_t + sigma_fpw - sigma_u,
                       tt_.event_mam(u, 0, event_type::kDep).mam_, theta);
#endif
            // recheck theta
            if (theta.any()) {
#ifndef NDEBUG
              ...
              // add expanded transfer to set
              expanded_transfers_[t.v_][neighbor.stop_idx_from_].emplace_back(
                  theta, u, neighbor.stop_idx_to_, sigma_fpw);

              ++prepro_stats_.n_transfers_initial_;
              ...
              for (stop_idx_t j_prime = neighbor.stop_idx_to_ + 1U;
                ...
                if (theta_prime.any()) {
                  ...
                  rlb.transports_[j_prime].emplace_back(
                      sigma_t + sigma_fpw - sigma_u,
                      tt_.event_mam(u, 0, event_type::kDep).mam_, theta_prime);
#endif
                }
              }
            }
      ...
      // reverse iteration
      for (auto i = static_cast<stop_idx_t>(stop_seq_from.size() - 1U); i != 0U;
           --i) {
        ...
        std::uint16_t const tau_arr_t_i = static_cast<std::uint16_t>(
            tt_.event_mam(t, i, event_type::kArr).count());

        // time of day for tau_arr(t,i)
        std::uint16_t const alpha = tt_.event_mam(t, i, event_type::kArr).mam_;

        // init the reached reduction data structure
#ifdef TB_MIN_WALK
        rr_arr.update_walk(p_t_i, tau_arr_t_i, 0, beta_t, nullptr);
        rr_ch.update_walk(
            p_t_i, tau_arr_t_i + tt_.locations_.transfer_time_[p_t_i].count(),
            0, beta_t, nullptr);
        for (auto const& fp : tt_.locations_.footpaths_out_[p_t_i]) {
        ...
        for (auto const& fp : tt_.locations_.footpaths_out_[p_t_i]) {
          rr_arr.update_class(fp.target(), tau_arr_t_i + fp.duration_, 0,
                              beta_t, nullptr);
          rr_ch.update_class(fp.target(), tau_arr_t_i + fp.duration_, 0, beta_t,
                             nullptr);
        }
#else
        rr_arr.update(p_t_i, tau_arr_t_i, beta_t, nullptr);
        rr_ch.update(p_t_i,
                     tau_arr_t_i + tt_.locations_.transfer_time_[p_t_i].count(),
                     beta_t, nullptr);
        for (auto const& fp : tt_.locations_.footpaths_out_[p_t_i]) {
        ...
        for (auto transfer = expanded_transfers_[t.v_][i].begin();
             transfer != expanded_transfers_[t.v_][i].end();) {
          impr.reset();

          // get route of transport that we transfer to
          auto const route_u =
              tt_.transport_route_[transfer->transport_idx_to_];

          // convert departure into timescale of transport t
          auto const tau_dep_u_j =
              tt_.event_mam(transfer->transport_idx_to_, transfer->stop_idx_to_,
                            event_type::kDep);
          ...
          for (stop_idx_t l = transfer->stop_idx_to_ + 1U;
            ...
            std::uint16_t const tau_arr_t_u_l =
                tau_dep_t_u_j + static_cast<std::uint16_t>(
                                    tt_.event_mam(transfer->transport_idx_to_,
                                                  l, event_type::kArr)
                                        .count() -
                                    tau_dep_u_j.count());
            ...
            auto const p_u_l =
                stop{tt_.route_location_seq_[route_u][l]}.location_idx();

#if defined(TB_MIN_WALK) || defined(TB_TRANSFER_CLASS)
            // target location of the transfer
            auto const p_u_j =
                stop{tt_.route_location_seq_[route_u][transfer->stop_idx_to_]}
                    .location_idx();
#endif
            ...
            rr_arr.update_walk(p_u_l, tau_arr_t_u_l, walk_time_l, transfer->bf_,
                               &impr);
            rr_ch.update_walk(
                p_u_l,
                tau_arr_t_u_l + tt_.locations_.transfer_time_[p_u_l].count(),
                walk_time_l, transfer->bf_, &impr);
#elifdef TB_TRANSFER_CLASS
            std::uint16_t walk_or_change = 0;
            if (p_t_i == p_u_j) {
              walk_or_change = tt_.locations_.transfer_time_[p_t_i].count();
            } else {
              for (auto const& fp : tt_.locations_.footpaths_out_[p_t_i]) {
                if (fp.target() == p_u_j) {
            ...
            rr_ch.update(
                ...
                transfer->bf_, &impr);
#endif
            for (auto const& fp_r : tt_.locations_.footpaths_out_[p_u_l]) {
              std::uint16_t const eta = tau_arr_t_u_l + fp_r.duration_;
#ifdef TB_MIN_WALK
              std::uint16_t const walk_time_r =
                  matches(tt_, location_match_mode::kEquivalent, p_u_l,
                          fp_r.target())
                      ? walk_time_l
                      : walk_time_l + fp_r.duration_;
              rr_arr.update_walk(fp_r.target(), eta, walk_time_r, transfer->bf_,
                                 &impr);
              rr_ch.update_walk(fp_r.target(), eta, walk_time_r, transfer->bf_,
                                &impr);
#elifdef TB_TRANSFER_CLASS
          ...
          if (transfer->bf_.none()) {
            // remove it
            transfer = expanded_transfers_[t.v_][i].erase(transfer);
            ++prepro_stats_.n_transfers_reduced_;
          } else {
            ++transfer;
          }
        }
