...
void tb_preprocessor::reduce(
    std::vector<std::vector<expanded_transfer>>& expanded_transfers) {
  for (std::size_t stop_idx = 0U; stop_idx != expanded_transfers.size();
       ++stop_idx) {
    for (auto const& expanded_transfer : expanded_transfers[stop_idx]) {
      reduced_transfers_[stop_idx].emplace_back(
          get_or_create_bfi(expanded_transfer.bf_).v_,
          expanded_transfer.transport_idx_to_, expanded_transfer.stop_idx_to_,
          expanded_transfer.passes_midnight_);
      ++n_transfers_;
    }
  }
}
...
#pragma omp parallel default(none) \
    shared(ts, tt_, reduced_transfers_, progress_tracker) num_threads(4)
    {
#ifdef TB_PREPRO_TRANSFER_REDUCTION
      ...
      std::vector<std::vector<expanded_transfer>> transfers;
...
#pragma omp for ordered
      for (std::uint32_t t_uint = 0U;
           t_uint != tt_.transport_traffic_days_.size(); ++t_uint) {
        transport_idx_t const t{t_uint};
        ...
        for (auto i = static_cast<stop_idx_t>(stop_seq_t.size() - 1U); i != 0U;
             --i) {
#else
        for (auto i = static_cast<stop_idx_t>(1U); i != stop_seq_t.size();
             ++i) {
#endif
          ...
          auto handle_fp = [&sigma_t, this, &t, &i, &route_t, &transfers, &alpha
            ...
            for (auto const route_u : routes_at_q) {
              ...
              for (stop_idx_t j = 0U;
                ...
                if (p_u_j == q&& stop{tt_.route_location_seq_[route_u][j]}
                                 .in_allowed()) {
                  ...
                  while (omega.any()) {
                    ...
                    auto const u = tt_.route_transport_ranges_
                                       [route_u][static_cast<std::size_t>(k)];
                      ...
                      if (theta.any()) {
                        ...
                        if (!check_uturn()) {
                          ...
                          if (theta.any()) {
                            ...
                            transfers[i].emplace_back(theta, u.v_, j, sigma);
...
#pragma omp ordered
        {
          ...
          // add reduced transfers of this transport to the transfer set
          ts.data_.emplace_back(
              it_range{reduced_transfers_.cbegin(),
                       reduced_transfers_.cbegin() +
                           static_cast<std::int64_t>(stop_seq_t.size())});

          // clear reduced transfers
          clear_reduced_transfers(stop_seq_t.size());
