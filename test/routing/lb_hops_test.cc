...
TEST(routing, lb_hops) {
  ...
  auto location_hops = vector_map<location_idx_t, std::uint8_t>{};

  lb_hops<direction::kForward>(tt, q, location_hops);

  EXPECT_EQ(location_hops[tt.find(location_id{"T", source_idx_t{}}).value()],
            0U);
  EXPECT_EQ(location_hops[tt.find(location_id{"S", source_idx_t{}}).value()],
            1U);
  EXPECT_EQ(location_hops[tt.find(location_id{"B1", source_idx_t{}}).value()],
            1U);
  EXPECT_EQ(location_hops[tt.find(location_id{"C2", source_idx_t{}}).value()],
            1U);
  EXPECT_EQ(location_hops[tt.find(location_id{"D3", source_idx_t{}}).value()],
            1U);
  EXPECT_EQ(location_hops[tt.find(location_id{"F", source_idx_t{}}).value()],
            1U);
  EXPECT_EQ(location_hops[tt.find(location_id{"P", source_idx_t{}}).value()],
            2U);
  EXPECT_EQ(location_hops[tt.find(location_id{"C1", source_idx_t{}}).value()],
            2U);
  EXPECT_EQ(location_hops[tt.find(location_id{"D2", source_idx_t{}}).value()],
            2U);
  EXPECT_EQ(location_hops[tt.find(location_id{"D1", source_idx_t{}}).value()],
            3U);
}